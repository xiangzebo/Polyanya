...
private:
	mutable BidirectionalDijkstraGPPC<TimestampFlags, AdjGraph> dij;
	int max_step_count;
public:
...
public:
	typedef typename type_of_arc<ArcHeadGPPC>::type Arc;

	DynAdjGraph<ArcHeadGPPC> forward_graph, backward_graph;
...
struct ContractionArcGPPC{
	int source, target, weight, mid, hop_length;
};

struct ContractionArcHeadGPPC{
	int target, weight, mid, hop_length;
};

inline
ContractionArcHeadGPPC get_head(ContractionArcGPPC a){
	return {a.target, a.weight, a.mid, a.hop_length};
}

inline
ContractionArcGPPC make_arc(int source, ContractionArcHeadGPPC h){
...
std::vector<int>compute_online_contraction_order(
	...
	auto core = make_contraction_core(
		node_count, map_range(
			arc_list, 
			[](WeightedArcGPPC a){
				return ContractionArcGPPC{a.source, a.target, a.weight, -1, 1};
			}
		)
	);
	...
	auto contract = [&](int v){
		for(auto b:core.backward_graph.out(v)){
			...
			on_backward_dag(WeightedArcGPPC{v, b.target, b.weight}, b.mid);
		}

		for(auto f:core.forward_graph.out(v)){
			level[f.target] = std::max(level[f.target], level[v]+1);
			++removed_neighbours[f.target];
			on_forward_dag(WeightedArcGPPC{v, f.target, f.weight}, f.mid);
		}
		...
		core.contract_node(v,
			[&](ContractionArcGPPC a){
				bool ret = do_only_longer_path_exist(
					...
					a.target,
					a.weight,
					[](ContractionArcGPPC a){return a.weight;}
				);
				return ret;
			},
			[=](ContractionArcGPPC l, ContractionArcGPPC r){
				return ContractionArcGPPC{
					l.source, 
					r.target, 
					l.weight + r.weight, 
					v,

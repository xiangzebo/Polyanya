...
public:
	...
	explicit kway_min_id_heap(int id_count, key_order_type order = key_order_type()):
		heap_end(0), heap(id_count), id_pos(id_count, -1), order(std::move(order)), contained_flags(id_count){
		check_id_invariants();
	...
	void reset(int new_id_count = 0, key_order_type new_order = key_order_type()){
		...
		contained_flags = TimestampFlags(new_id_count);
		clear();
	...
	bool contains(int id)const{
		...
		return contained_flags.is_set(id);
	}
	...
	int pop(){
		...
		if(heap_end == 1){
			heap_end = 0;
			id_pos[heap[0].id] = -1;
			contained_flags.unset(heap[0].id);
...
private:
	...
	TimestampFlags contained_flags;
	...
	void check_id_invariants()const{
		...
		for(int i=0; i<heap_end; ++i){
			...
			assert(contained_flags.is_set(heap[i].id));
			assert(id_pos[heap[i].id] == i);
		}

		for(int i=0; i<(int)id_pos.size(); ++i){
			if(contained_flags.is_set(i)){
				assert(0 <= id_pos[i]);

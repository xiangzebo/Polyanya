...
PathFindingAlgorithm * getAlgorithmType(AlgorithmType algType,std::vector<bool> &bits, char * mapData, int mapWidth, int mapHeight)
{
	if (algType == AT_BL_JPS_EXP2)
		return (new BL_JPS_Experimental_2(mapData, mapWidth, mapHeight));
	return 0;
}
...
void PreprocessMap(std::vector<bool> &bits, int width, int height, const char *filename)
{
    ...
    currentAlgorithm=getAlgorithmType(AT_BL_JPS_EXP2,bits,c,width,height);
...
void *PrepareForSearch(std::vector<bool> &bits, int w, int h, const char *filename)
{
    ...
    if (currentAlgorithm==0)
	{
        ...
        for (int i =0;i<bits.size();i++)
            c[i/8]|= (bits[i]!=1?1:0)<<(i&7);
        currentAlgorithm=getAlgorithmType(AT_BL_JPS_EXP2,bits,c,w,h);

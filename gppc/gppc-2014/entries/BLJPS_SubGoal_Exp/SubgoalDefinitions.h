...
typedef float cost;
#define INFINITE_COST std::numeric_limits<float>::infinity()
#else
typedef unsigned int cost;
//#define INFINITE_COST std::numeric_limits<unsigned int>::infinity()
...
public:
	...
	{
		subgoalId id;
		unsigned char dir, dir2;
		unsigned char followUpDirs;
		subGoalIdDir(subgoalId _id, unsigned char _dir) : id(_id), dir(_dir) { dir2 = 8; followUpDirs = 0xFF; }
		subGoalIdDir(subgoalId _id, unsigned char _dir, unsigned  char _dir2, unsigned char _followUpDirs) : id(_id), dir(_dir), dir2(_dir2), followUpDirs(_followUpDirs){		}
...
struct heapElement
{
	...
	cost fVal;
	char dir;
	unsigned char followUpDir;
	heapElement(subgoalId _sg, cost _fVal, char _dir, unsigned  char _followUpDir) : sg(_sg), fVal(_fVal), dir(_dir), followUpDir(_followUpDir){}
};
struct heapElement3
{
	subgoalId sg;
	cost fVal;
	heapElement3(subgoalId _sg, cost _fVal) : sg(_sg), fVal(_fVal){}
};

...
#endif
{
	...
	int useTwoLevel = 0;	// Use the Simple subgoal graph or the Two-level subgoal graph?
...
void SubgoalGraph::FinalizeGraph()
{
	...
	for (subgoalId sg = 0; sg < nSubgoals; sg++)
	{
		...
		neighbors[sg] = new subgoalId[nNeighbors[sg]+2];	// Allocate space for all of sg's neighbors + 1 for use during search
...
void SubgoalGraph::LoadGraph(const char *filename)
{
	...
	for (subgoalId sg = 0; sg < nSubgoals; sg++)
	{
		neighbors[sg] = new subgoalId[nNeighbors[sg] + 2];	// + 1 for the extra edge during search
...
cost SubgoalGraph::SubgoalAStarSearch(subgoalId & start, subgoalId & goal, cost searchLimit, std::vector<subgoalId> & abstractPath)
{
	...
	//if (search >= MAX_SEARCH)
		//ResetSearch();
/*
	for (uint i=0; i<(nSubgoals+2); i++){
		gCost[i]=INFINITE_COST;
		//SetClosed(i);
	}
*/
	...
	//SetOpen(start);
	AddToOpen(start, tBreak*HCost(start,goal));
...
#else
	while(!theHeap.empty() && generated[goal]!=search)	// 0-1 Expansions per loop
#endif
	{
		...
		for (unsigned int i = 0; i < nSuccessors; i++)
		{
			...
			if(generated[succ] != search)
			{
				...
				#ifdef USE_STACK
				if (gCost[succ] + tBreak*HCost(succ,goal) == currFCost)
					theStack.push_back(heapElement(succ, gCost[succ] + tBreak*HCost(succ,goal)));
				else
					AddToOpen(succ, gCost[succ] + tBreak*HCost(succ,goal));
				#else
				AddToOpen(succ, gCost[succ] + tBreak*HCost(succ,goal));
				#endif
			}
	...
	if (gCost[goal] != searchLimit)
	{
		...
		while (cur != start)
		{
			...
			if (finalized)	// If the graph is finalized, use 'neighbors' to get the successors
			{
				successors = neighbors[cur];
				nSuccessors = nNeighbors[cur];
			}
			else	// Else, use 'edgeVector' to get the successors
			{
				successors = edgeVector[cur].data();
				nSuccessors = edgeVector[cur].size();
			}
			std::vector <float> gScoresNeighbors;
			for (unsigned int i = 0; i < nSuccessors; i++)
			{
				if(generated[successors[i]] != search)
					gCost[successors[i]]=INFINITE_COST;
				gScoresNeighbors.push_back(gCost[successors[i]]);
			}
			int posMinGScore=distance(gScoresNeighbors.begin(), min_element(gScoresNeighbors.begin(), gScoresNeighbors.end()));
			cur=successors[posMinGScore];
			abstractPath.push_back(cur);
		}
		...
		std::vector<subgoalId> & startDirectHReachableSubgoals, std::vector<subgoalId> & goalDirectHReachableSubgoals, std::vector<subgoalId> & extraEdgeList)
{
	...
	else	// Create a new subgoal for it, with id nSubgoals (at the end of all the actual subgoals)
	{
		...
		for (unsigned int i = 0; i < startDirectHReachableSubgoals.size(); i++)
		{
			...
			subgoalId sg = startDirectHReachableSubgoals[i];

			neighbors[sg][nNeighbors[sg]] = sgStart;
			nNeighbors[sg]++;
			//SetExtraEdgeFlag(sg);
	...
	else	// Create a new subgoal for it, with id nSubgoals+1 (at the end of all the actual subgoals, after the start)
	{
		...
		neighbors[sgGoal] = goalDirectHReachableSubgoals.data();
